...
namespace LA
{
	...
	bool AnalyzeToken(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		...
		{
		case LEX_COMMA:
			lexTable.Add({ LEX_COMMA ,str_number, LT_TI_NULLXDX });
			return true;
		case LEX_LEFTBRACE:
			lexTable.Add({ LEX_LEFTBRACE,str_number,LT_TI_NULLXDX });
			return true;
		case LEX_BRACELET:
			lexTable.Add({ LEX_BRACELET,str_number,LT_TI_NULLXDX });
			return true;
		case LEX_LEFTHESIS:
			lexTable.Add({ LEX_LEFTHESIS,str_number,LT_TI_NULLXDX });
			return true;
		case LEX_RIGHTHESIS:
			lexTable.Add({ LEX_RIGHTHESIS,str_number,LT_TI_NULLXDX });
			return true;
		case LEX_SEMICOLON:
			lexTable.Add({ LEX_SEMICOLON,str_number,LT_TI_NULLXDX });
			return true;
		case LEX_EQUAL_SIGN:
			lexTable.Add({ LEX_EQUAL_SIGN, str_number, LT_TI_NULLXDX });
			return true;

		case '+':
			lexTable.Add({ LEX_PLUS,str_number,idTable.size - 1 });
			return true;
		case '-':
			lexTable.Add({ LEX_MINUS, str_number, idTable.size - 1 });
			return true;
		case '*':
			lexTable.Add({ LEX_STAR, str_number, idTable.size - 1 });
			return true;
		case '/':
			lexTable.Add({ LEX_DIRSLASH, str_number, idTable.size - 1 });
			return true;
		...
		case 'f':
		{
			...
			{
				lexTable.Add({ LEX_FUNCTION, str_number, LT_TI_NULLXDX });
				return true;
			}
			else
		...
		case 'i':
		{
			...
			{
				lexTable.Add({ LEX_INTEGER, str_number, LT_TI_NULLXDX });
		...
		case 's':
		{
			...
			{
				lexTable.Add({ LEX_STRING, str_number, LT_TI_NULLXDX });
		...
		case 'd':
		{
			...
			{
				lexTable.Add({ LEX_DECLARE, str_number, LT_TI_NULLXDX });
				return true;
			}
			else
				return AnalyzeIdentifier(token, str_number, lexTable, idTable, flag_type_variable);
		...
		case 'r':
		{
			...
			{
				lexTable.Add({ LEX_RETURN, str_number, LT_TI_NULLXDX });
				return true;
			}
			else
				return AnalyzeIdentifier(token, str_number, lexTable, idTable, flag_type_variable);
		...
		case 'p':
		{
			...
			{
				lexTable.Add({ LEX_PRINT, str_number, LT_TI_NULLXDX });
				return true;
			}
			else
				return AnalyzeIdentifier(token, str_number, lexTable, idTable, flag_type_variable);
		...
		case '\'':
		{
			...
			if (FST::execute(stringLiteral))
			{
				...
				if (i != LT_TI_NULLXDX)
					lexTable.Add({ LEX_LITERAL, str_number, i });
				else
				{
					...
					lexTable.Add({ LEX_LITERAL, str_number,idTable.size - 1 });
				}
				return true;
			}
		}
	...
	bool AnalyzeIdentifier(char* token, int str_number, LT::LexTable& lexTable, IT::IdTable& idTable, TypeOfVar& flagTypeVariable)
	{
			...
			&& lexTable.GetEntry(lexTable.size - 2).lexema == LEX_DATATYPE)
		{
			...
			lexTable.Add({ LEX_ID, str_number, idTable.size - 1 });
		...
		for (int i = lexTable.size - 1; i >= 0; i--)
		{
				...
				&& idTable.GetEntry(lexTable.GetEntry(i).idxTI).idtype == IT::IDTYPE::F)
			{
				...
				if (temp != -1)
				{
					lexTable.Add({ LEX_ID,str_number,temp });
					return true;
				}
				else
				{
